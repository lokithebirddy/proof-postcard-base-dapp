...
const source = await readFile(resolve(process.cwd(), "contracts/ProofPostcard.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "ProofPostcard.sol": { content: source } },
  settings: { outputSelection: { "*": { "*": ["abi", "evm.bytecode.object"] } } },
};
const output = JSON.parse(solc.compile(JSON.stringify(input)));
const contract = output.contracts?.["ProofPostcard.sol"]?.ProofPostcard;
...
console.log(`Deploying ProofPostcard from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_PROOF_POSTCARD_CONTRACT_ADDRESS to this contract address in Vercel.");
